  ...
  try {
    ...
    db.Users.findOne(
      ...
      (err, user) => {
        ...
        if (user) {
          var file_name =
            user_id + "/" + Date.now() + "_" + req.file.originalname;
          minio.minioClient.putObject(
            "files",
            file_name,
            req.file.buffer,
            function (err, file) {
    ...
    db.Users.findOne(
      ...
      (err, user) => {
        ...
        if (user) {
          ...
          var itemList = [];
          var stream = minio.minioClient.listObjects(
            "files",
            folder_name,
            false
          );
...
function downloadFile(req, res) {
  try {
    ...
    minio.minioClient.getObject("files", req.query.file_url, function (
      err,
      stream
    ) {
      if (err) {
